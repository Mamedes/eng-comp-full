...
import type { Artist, Album, PaginatedResponse, ArtistFormData, AlbumFormData } from './types'
...
export async function getArtists(
  ...
  searchParams.set('perPage', (params.perPage || 30).toString())
  searchParams.set('sort', params.sort || 'nome')
  searchParams.set('dir', params.dir || 'desc')

  // Certifique-se que o endpoint aqui é "/artista" conforme seu serviço
  return fetchApi<PaginatedResponse<Artist>>(`/artista?${searchParams.toString()}`, {}, token)
}

export async function getArtistById(id: number, token?: string | null): Promise<Artist> {
  return fetchApi<Artist>(API_ENDPOINTS.artistById(id), {}, token)
}

export async function createArtist(data: ArtistFormData, token?: string | null): Promise<Artist> {
...
export async function getAlbumById(id: number, token?: string | null): Promise<Album> {
  return fetchApi<Album>(API_ENDPOINTS.albumById(id), {}, token)
}

export async function createAlbum(data: AlbumFormData, token?: string | null): Promise<Album> {
