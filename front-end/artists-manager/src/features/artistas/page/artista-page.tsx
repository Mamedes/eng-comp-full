                ...
                <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                    <div>
                        <h1 className="text-4xl font-black tracking-tighter sm:text-5xl flex items-center gap-3">
                            <Users className="text-purple-500" /> Artistas
                        </h1>
                        <p className="text-zinc-500 font-medium mt-2">Gerencie sua base de talentos.</p>
                    </div>
                ...
                <div className="max-w-[1600px] mx-auto py-6">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-32 gap-4">
                            ...
                            <span className="text-zinc-500 font-medium animate-pulse">Carregando artistas...</span>
                        </div>
                    ) : data.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
                            {data.map((artista) => (
                                <div key={artista.artistaId}
                                    ...
                                    className="cursor-pointer group relative bg-zinc-900/40 border border-white/5 p-6 rounded-3xl hover:bg-zinc-900/80 hover:border-purple-500/30 transition-all hover:-translate-y-1 shadow-xl">
                ...
                <footer className="px-8 py-6 bg-zinc-950/80 backdrop-blur-md border-t border-white/5 flex-shrink-0">
                    <div className="max-w-[1600px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                        <span className="text-sm text-zinc-500">
                            Mostrando <span className="text-white font-bold">{data.length}</span> de <span className="text-white font-bold">{pagination.total}</span> artistas
